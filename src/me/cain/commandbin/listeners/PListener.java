...
public class PListener extends PlayerListener
{
    ...
    if (e.getAction() == Action.RIGHT_CLICK_AIR)
    {
      if (CommandBin.plugin.getConfig().getBoolean(e.getPlayer().getName() + ".explosionstick"))
      {
        if (e.getPlayer().getItemInHand().getType() == Material.STICK)
        {
          ...
          e.getPlayer().getWorld().createExplosion(block, 8.0F);
        }
      }
  ...
  public void onPlayerLogin(PlayerLoginEvent e)
  {
    if (CommandBin.plugin.getConfig().getBoolean(e.getPlayer().getName() + ".banned"))
    {
      ...
      e.disallow(PlayerLoginEvent.Result.KICK_BANNED, "You were banned for: " + ChatColor.GRAY + CommandBin.plugin.getConfig().get(new StringBuilder(String.valueOf(e.getPlayer().getName())).append(".banreason").toString()).toString());
    }
  }
